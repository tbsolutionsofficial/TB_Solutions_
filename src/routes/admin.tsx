...
import {
  LayoutDashboard, Briefcase, Layers, Image as ImageIcon, MessageSquare,
  Mail, Star, Tag, FileText, LogOut, Plus, Trash2, Search, Loader2, Pencil, ImagePlus, X,
} from "lucide-react";
import {
...
import { COLLECTIONS, useFirestoreCollection, useFirestoreMutations, type MediaAsset } from "@/lib/firestore";
import { uploadToCloudinary } from "@/lib/cloudinary";
import { Dialog, DialogContent, DialogHeader, DialogTitle, DialogFooter } from "@/components/ui/dialog";
import { adminUser as mockAdminUser, chartData, mockContacts, mockOffers, mockMedia } from "@/mock/admin";
...
import { galleryItems as seedGallery, galleryCategories, type GalleryItem } from "@/content/gallery";
...
type Tab = "dashboard" | "projects" | "domains" | "gallery" | "testimonials" | "contacts" | "offers" | "media" | "content";
...
function AdminPage() {
  ...
  const galleryQuery = useFirestoreCollection<GalleryItem>(COLLECTIONS.gallery, { initialData: seedGallery });
  const testimonialsQuery = useFirestoreCollection<Testimonial>(COLLECTIONS.reviews, { initialData: seedTestimonials });
  const projectsQuery = useFirestoreCollection<Project>(COLLECTIONS.projects, { initialData: seedProjects });
  ...
  const domainRows = (domainsQuery.data ?? []).map((d) => ({ id: d.id, title: d.title, slug: d.slug, order: d.order, featured: d.featured ? "yes" : "no" }));
  const testimonialRows = (testimonialsQuery.data ?? []).map((t) => ({ id: t.id, name: t.name, company: t.company, stars: t.stars, status: t.status }));
  const contactRows = (contactsQuery.data ?? []).map((c) => ({ id: c.id, name: c.name, email: c.email, domain: c.domain ?? "-", read: c.read ? "yes" : "no" }));
  ...
  return (
    <div className="flex min-h-dvh bg-cream">
      <aside className="sticky top-0 hidden h-dvh w-64 shrink-0 flex-col border-r border-border bg-warm-white/80 backdrop-blur lg:flex">
        <div className="p-6">
          <Link to="/" className="font-display text-xl font-bold text-foreground">TB_Solutions</Link>
        ...
        <div className="border-t border-border p-4">
          <div className="flex items-center gap-3">
            ...
            <button onClick={() => auth.signOut()} className="text-muted-foreground hover:text-copper-dark" aria-label="Log out">
      ...
      <main className="flex-1 overflow-x-hidden">
        ...
        <div className="p-6">
          ...
          {tab === "projects" && (
            <ProjectsAdmin projects={projectsQuery.data ?? []} domains={domainsQuery.data ?? []} mutations={projectsMut} />
          )}
          {tab === "domains" && (
            <CrudTable
              title="Domains"
              ...
              rows={domainRows}
              blankRow={{ title: "New", slug: `new-${Date.now()}`, order: domainRows.length + 1, featured: "no" }}
          ...
          {tab === "gallery" && <GalleryAdmin items={galleryQuery.data ?? []} mutations={galleryMut} />}
          {tab === "testimonials" && (
            <CrudTable
              title="Testimonials"
          ...
          {tab === "offers" && <CrudTable title="Offers" seed={mockOffers.map((o) => ({ id: o.id, title: o.title, discount: `${o.discount}%`, active: o.active ? "yes" : "no" }))} columns={["title", "discount", "active"]} />}
          {tab === "media" && (
            <MediaGrid
              items={mediaQuery.data ?? []}
              onUpload={(asset) => mediaMut.create.mutate(asset as unknown as Record<string, unknown>)}
...
function AdminLoginGate({ signIn }: { signIn: (email: string, password: string) => Promise<unknown> }) {
...
function DashboardView({ stats }: { stats: { totalProjects: number; pendingReviews: number; unreadContacts: number; activeBanners: number } }) {
  const cards = [
        ...
        <div className="rounded-2xl bg-warm-white p-6 shadow-sm">
          ...
          <ResponsiveContainer width="100%" height={240}>
            <LineChart data={chartData}>
              ...
              <YAxis fontSize={12} />
              <Tooltip />
              <Line type="monotone" dataKey="projects" stroke="#bf572c" strokeWidth={2.5} dot={{ r: 4 }} />
            </LineChart>
          </ResponsiveContainer>
        </div>
        <div className="rounded-2xl bg-warm-white p-6 shadow-sm">
...
interface Row { id: string; [k: string]: string | number | boolean | undefined }
...
function CrudTable({
  ...
  const filtered = data.filter((r) => Object.values(r).some((v) => String(v).toLowerCase().includes(q.toLowerCase())));
      ...
      <div className="overflow-x-auto">
        <table className="w-full text-left text-sm">
          ...
          <tbody>
                  ...
                  <td key={c} className="py-3 pr-4">
                    <input
                      ...
                      onBlur={(e) => {
                        ...
                        if (isLocalMode) {
                          setLocalRows(localRows.map((row) => (row.id === r.id ? { ...row, [c]: e.target.value } : row)));
                        } else {
            ...
            {filtered.length === 0 && (
              <tr><td colSpan={columns.length + 1} className="py-8 text-center text-muted-foreground">No results</td></tr>
            )}
          </tbody>
        </table>
      </div>
...
function ProjectFormModal({
  ...
  useEffect(() => {
    if (open) setValues(toProjectFormValues(initial));
  }, [initial, open]);
  ...
  const addExtraLink = () => setValues((v) => ({ ...v, extraLinks: [...v.extraLinks, { label: "", url: "" }] }));
  const updateExtraLink = (i: number, patch: Partial<{ label: string; url: string }>) =>
    setValues((v) => ({ ...v, extraLinks: v.extraLinks.map((l, idx) => (idx === i ? { ...l, ...patch } : l)) }));
  const removeExtraLink = (i: number) =>
    setValues((v) => ({ ...v, extraLinks: v.extraLinks.filter((_, idx) => idx !== i) }));
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const extraLinks = values.extraLinks.filter((l) => l.label.trim() && l.url.trim());
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-h-[90vh] max-w-2xl overflow-y-auto bg-warm-white">
        <DialogHeader>
          <DialogTitle className="font-display">{initial ? "Edit project" : "Add project"}</DialogTitle>
        </DialogHeader>
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            ...
            <div className="flex items-center gap-4">
              {values.image && <img src={values.image} alt="" className="h-16 w-16 rounded-lg object-cover" />}
              <input
                ref={fileInputRef}
                type="file"
                accept="image/*"
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            <Field label="Title">
              <input required value={values.title} onChange={(e) => setValues((v) => ({ ...v, title: e.target.value }))} className={inputClass} />
            </Field>
            <Field label="Tag">
              <input value={values.tag} onChange={(e) => setValues((v) => ({ ...v, tag: e.target.value }))} className={inputClass} placeholder="e.g. Artificial Intelligence" />
            </Field>
          </div>

          <Field label="Domain">
            <select value={values.domain} onChange={(e) => setValues((v) => ({ ...v, domain: e.target.value }))} className={inputClass}>
          ...
          <Field label="Short description">
            <textarea rows={2} value={values.description} onChange={(e) => setValues((v) => ({ ...v, description: e.target.value }))} className={inputClass} />
          </Field>
          <Field label="Full description">
            <textarea rows={4} value={values.fullDescription} onChange={(e) => setValues((v) => ({ ...v, fullDescription: e.target.value }))} className={inputClass} />
          </Field>

          <div className="grid gap-4 sm:grid-cols-2">
            <Field label="Tech stack (comma-separated)">
              <input value={values.techStack} onChange={(e) => setValues((v) => ({ ...v, techStack: e.target.value }))} className={inputClass} placeholder="React, Node, Postgres" />
            </Field>
            <Field label="Tags (comma-separated)">
              <input value={values.tags} onChange={(e) => setValues((v) => ({ ...v, tags: e.target.value }))} className={inputClass} placeholder="AI, RAG" />
            </Field>
          </div>

          <div className="grid gap-4 sm:grid-cols-3">
            <Field label="Client">
              <input value={values.client} onChange={(e) => setValues((v) => ({ ...v, client: e.target.value }))} className={inputClass} />
            </Field>
            <Field label="Completion date">
              <input type="date" value={values.completionDate} onChange={(e) => setValues((v) => ({ ...v, completionDate: e.target.value }))} className={inputClass} />
            </Field>
            <Field label="Status">
              <select value={values.status} onChange={(e) => setValues((v) => ({ ...v, status: e.target.value as ProjectFormValues["status"] }))} className={inputClass}>
          ...
          <div className="flex items-center gap-6">
            <label className="flex items-center gap-2 text-sm text-foreground">
              <input type="checkbox" checked={values.featured} onChange={(e) => setValues((v) => ({ ...v, featured: e.target.checked }))} />
              Featured
            </label>
            <div className="w-28">
              <Field label="Sort order">
                <input type="number" value={values.sortOrder} onChange={(e) => setValues((v) => ({ ...v, sortOrder: Number(e.target.value) }))} className={inputClass} />
              </Field>
            </div>
          </div>
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            <Field label="GitHub repo">
              <input value={values.github} onChange={(e) => setValues((v) => ({ ...v, github: e.target.value }))} className={inputClass} placeholder="https://github.com/..." />
            </Field>
            <Field label="Live site / demo">
              <input value={values.website} onChange={(e) => setValues((v) => ({ ...v, website: e.target.value }))} className={inputClass} placeholder="https://..." />
            </Field>
          </div>
          ...
          <div>
            <div className="flex items-center justify-between">
              ...
              <button type="button" onClick={addExtraLink} className="text-xs font-semibold text-copper-dark hover:underline">
                + Add link
              </button>
            </div>
            <div className="mt-2 space-y-2">
              {values.extraLinks.map((link, i) => (
                <div key={i} className="flex gap-2">
                  ...
                  />
                  <button type="button" onClick={() => removeExtraLink(i)} className="rounded p-2 text-muted-foreground hover:bg-destructive/10 hover:text-destructive" aria-label="Remove link">
                    <X className="h-4 w-4" />
                  </button>
                </div>
              ))}
              {values.extraLinks.length === 0 && (
                <p className="text-xs text-muted-foreground">No extra links yet — add a YouTube demo, resources page, Figma file, or anything else.</p>
              )}
            </div>
          </div>

          <DialogFooter>
            <button type="button" onClick={() => onOpenChange(false)} className="rounded-full border border-border px-5 py-2 text-sm font-semibold text-foreground hover:bg-secondary">
              Cancel
            </button>
            <button type="submit" disabled={submitting} className="inline-flex items-center justify-center gap-2 rounded-full bg-primary px-5 py-2 text-sm font-semibold text-primary-foreground disabled:opacity-60">
...
function ProjectsAdmin({
  ...
  const handleSubmit = (data: Record<string, unknown>) => {
    if (editing) {
      mutations.update.mutate(
        { id: editing.id, data },
        { onSuccess: () => { toast.success("Project updated"); setModalOpen(false); } },
      );
    } else {
      mutations.create.mutate(data, {
        onSuccess: () => { toast.success("Project added"); setModalOpen(false); },
      });
    }
  };
  ...
  return (
    <div className="rounded-2xl bg-warm-white p-6 shadow-sm">
            ...
            {filtered.map((p) => {
              ...
              return (
                <tr key={p.id} className="border-b border-border/60 hover:bg-secondary/40">
                  ...
                  <td className="py-3 pr-4">
                    <span className="rounded-full bg-secondary px-2 py-0.5 text-xs font-medium text-copper-dark">{p.status}</span>
                  </td>
                  <td className="py-3 pr-4 text-xs text-muted-foreground">{linkLabels.length ? linkLabels.join(", ") : "—"}</td>
                  <td className="py-3 text-right">
                    <button onClick={() => openEdit(p)} className="rounded p-1.5 text-muted-foreground hover:bg-secondary hover:text-copper-dark" aria-label="Edit">
                      <Pencil className="h-4 w-4" />
                    </button>
                    <button
                      onClick={() => { mutations.remove.mutate(p.id); toast.success("Deleted"); }}
                      className="rounded p-1.5 text-muted-foreground hover:bg-destructive/10 hover:text-destructive"
                      aria-label="Delete"
                    >
                      <Trash2 className="h-4 w-4" />
      ...
      <ProjectFormModal open={modalOpen} onOpenChange={setModalOpen} initial={editing} domains={domains} onSubmit={handleSubmit} submitting={submitting} />
    </div>
  );
}
...
function GalleryFormModal({
  ...
  const handleUpload = async (file: File) => {
    ...
    try {
      ...
      setValues((v) => ({ ...v, src: result.url, type: result.resourceType === "video" ? "video" : "image" }));
    } catch (err) {
      console.error(err);
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    onSubmit({ title: values.title, category: values.category, type: values.type, src: values.src, span: values.span });
  };
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-lg bg-warm-white">
        <DialogHeader>
          <DialogTitle className="font-display">{initial ? "Edit gallery item" : "Add gallery photo"}</DialogTitle>
        </DialogHeader>
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            <span className="mb-1.5 block text-xs font-semibold uppercase tracking-wider text-muted-foreground">
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            <Field label="Category">
              <select value={values.category} onChange={(e) => setValues((v) => ({ ...v, category: e.target.value as GalleryItem["category"] }))} className={inputClass}>
                {categories.map((c) => (
            ...
            <Field label="Banner size">
              <select value={values.span} onChange={(e) => setValues((v) => ({ ...v, span: e.target.value as NonNullable<GalleryItem["span"]> }))} className={inputClass}>
          ...
          <DialogFooter>
            ...
            <button type="submit" disabled={submitting || uploading} className="inline-flex items-center justify-center gap-2 rounded-full bg-primary px-5 py-2 text-sm font-semibold text-primary-foreground disabled:opacity-60">
              {submitting ? <Loader2 className="h-4 w-4 animate-spin" /> : null}
...
function GalleryAdmin({ items, mutations }: { items: GalleryItem[]; mutations: ReturnType<typeof useFirestoreMutations> }) {
  const [modalOpen, setModalOpen] = useState(false);
  ...
  const handleSubmit = (data: Record<string, unknown>) => {
    if (editing) {
      mutations.update.mutate(
        { id: editing.id, data },
        { onSuccess: () => { toast.success("Updated"); setModalOpen(false); } },
      );
    } else {
      mutations.create.mutate(data, {
        onSuccess: () => { toast.success("Added"); setModalOpen(false); },
      });
    }
  };
  ...
  return (
    <div className="rounded-2xl bg-warm-white p-6 shadow-sm">
      ...
      <div className="grid grid-cols-2 gap-3 sm:grid-cols-4 lg:grid-cols-6">
            ...
            <div className="absolute inset-x-0 bottom-0 bg-espresso/80 p-2 text-xs text-warm-white opacity-0 transition-opacity group-hover:opacity-100">
              ...
              <div className="flex items-center justify-between text-warm-white/60">
                ...
                <div className="flex gap-2">
                  <button onClick={() => openEdit(item)} className="hover:text-copper-light" aria-label="Edit">
                    <Pencil className="h-3.5 w-3.5" />
                  </button>
                  <button
                    onClick={() => { mutations.remove.mutate(item.id); toast.success("Deleted"); }}
        ...
        {items.length === 0 && (
          <div className="col-span-full py-8 text-center text-muted-foreground">No gallery items yet</div>
        )}
      </div>
      <GalleryFormModal open={modalOpen} onOpenChange={setModalOpen} initial={editing} onSubmit={handleSubmit} submitting={submitting} />
    </div>
  );
}
...
function MediaGrid({
  ...
  return (
    <div className="rounded-2xl bg-warm-white p-6 shadow-sm">
      ...
      <div className="grid grid-cols-2 gap-3 sm:grid-cols-4 lg:grid-cols-6">
        {items.map((m) => (
          <div key={m.id} className="group relative aspect-square overflow-hidden rounded-xl border border-border">
            <img src={m.url} alt={m.name} className="h-full w-full object-cover" />
            <div className="absolute inset-x-0 bottom-0 bg-espresso/80 p-2 text-xs text-warm-white opacity-0 transition-opacity group-hover:opacity-100">
              <div className="truncate">{m.name}</div>
              <div className="flex items-center justify-between text-warm-white/60">
                <span>{m.sizeKb ?? 0} KB</span>
                <button onClick={() => onDelete(m.id)} className="hover:text-destructive" aria-label="Delete">
                  <Trash2 className="h-3.5 w-3.5" />
                </button>
              </div>
            </div>
  ...
  return (
    <div className="max-w-3xl space-y-4 rounded-2xl bg-warm-white p-6 shadow-sm">
      <div>
        ...
        <input value={hero} onChange={(e) => setHero(e.target.value)} className="w-full rounded-lg border border-border px-3 py-2 outline-none focus:border-copper" />
      </div>
      <div>
        <label className="mb-2 block text-sm font-semibold text-foreground">Hero description</label>
        <textarea rows={4} defaultValue="TB_Solutions is your project consultancy partner…" className="w-full rounded-lg border border-border px-3 py-2 outline-none focus:border-copper" />
      </div>
      <div>
        <label className="mb-2 block text-sm font-semibold text-foreground">Contact email</label>
        <input defaultValue="hello@tbsolutions.dev" className="w-full rounded-lg border border-border px-3 py-2 outline-none focus:border-copper" />
      </div>
      <button
